...
const DoughnutGraph = ({ doughnutData, dataKey, labelKey, colors }: { doughnutData: object[], dataKey: string, labelKey: string, colors: string[] }) => {
  ...
  return (
    <PieChart width={700} height={300}>
      <Pie
        ...
        activeShape={(props: any) => renderActiveShape(props, dataKey, labelKey)}
        outerRadius={110}
        fill="#bc3d70"
        paddingAngle={1}
        // dataKey="value"
      ...
      >
        {doughnutData.map((entry: any, index: number) => (
          <Cell key={`cell-${index}`} fill={colors[index % doughnutData.length]} />
          // <Cell key={`cell-${index}`} />
        ))}
      </Pie>
    </PieChart>
