...
const Requirements = ({
    ...
    return (
        <Box>
            <Text fontSize={"2xl"} mb={"20px"} color={"#D9D9D9"} textAlign={"left"}>Requirements</Text>
            <VStack color={"#409F4E"} fontSize={"1.2rem"} align='stretch'>
                ...
                {vetoVotes && <Box display={"flex"} color={
                    satisfiedBgMap[isVetoSatisfied as keyof typeof satisfiedBgMap]
                        .color} alignItems={"center"} gap={"10px"}>{satisfiedBgMap[isVetoSatisfied as keyof typeof satisfiedBgMap].icon}<Text>{isVetoSatisfied === "na" ? "N/A" : isYesSatisfied === "yes" ? "Less" : "More"} than 33.4% have voted 'Veto'.</Text></Box>}
